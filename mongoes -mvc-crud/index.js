...
app.put("/api/enquiry-update", async (req, res) => {
    ...
    // let updateResponse = await enquiryModel.updateOne({ _id: id }, { $set: updateRecord }); its working
    let updateResponse = await enquiryModel.updateOne({ _id: id }, updateRecord );
    res.send({
        status: 1,
        msg: "user api",
        updateResponse
